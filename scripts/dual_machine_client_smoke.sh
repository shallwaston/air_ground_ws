...
print_usage() {
  echo "Usage: ./scripts/dual_machine_client_smoke.sh [--check-only] [--env-file path/to/file.env] [additional launch arguments]"
  echo "Example: ./scripts/dual_machine_client_smoke.sh --env-file env/machine_b_default.env"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --env-file)
      if [[ $# -lt 2 ]]; then
        echo "[dual_machine_client_smoke] ERROR: --env-file requires a path."
        print_usage
        exit 1
      fi
      ENV_FILE="$2"
...
stage2_load_env_file "dual_machine_client_smoke" "${ENV_FILE}"
stage2_require_vars_from_env_file "dual_machine_client_smoke" "${ENV_FILE}" \
  STAGE2_MACHINE_ROLE WORKSPACE_PATH ROS_DOMAIN_ID RMW_IMPLEMENTATION
stage2_warn_workspace_path_mismatch "dual_machine_client_smoke" "${WORKSPACE_DIR}"

if [[ -n "${STAGE2_ENV_FILE_LOADED:-}" && -n "${ROS_DISCOVERY_SERVER:-}" ]]; then
  echo "[dual_machine_client_smoke] INFO: env file provided ROS_DISCOVERY_SERVER=${ROS_DISCOVERY_SERVER}, but default-discovery smoke will unset it."
fi

if [[ -n "${STAGE2_ENV_FILE_LOADED:-}" && "${ROS_LOCALHOST_ONLY:-0}" == "1" ]]; then
  echo "[dual_machine_client_smoke] INFO: env file set ROS_LOCALHOST_ONLY=1, but dual-machine smoke will unset it."
fi
